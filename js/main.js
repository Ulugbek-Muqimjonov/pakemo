...
let elweight = document.querySelector(".site-header__userSearch-egg");
let elheigth = document.querySelector(".site-header__userSearch-swapntime");
...
elForm.addEventListener("submit", function(evt) {
    ...
    let  elweightvalue = elweight.value.trim();
    let  elheighttimevalue = elheigth.value.trim();
    
    let mySearch = pokemons.filter(function (item,index,arr) {
        return item.name.toUpperCase() == elinpvalue;   
    })
 
    
    let mySearch2 = pokemons.filter(function (item,index,arr) {
        return item.weight == elweightvalue;   
    })
    
    
    let mySearch3 = pokemons.filter(function (item,index,arr) {
        return item.height == elheighttimevalue;   
    })
